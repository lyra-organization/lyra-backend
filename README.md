...
The trigger blocks new matches when either user already has a `pending`, `approved`, or `confirmed` match — one active match at a time.
...
| Met confirmation | Stores `met` status | `/respond-match` (action: `met`) | Fires on celebration (< 15m) |
...
└── supabase/
    ...
    │   └── 20260314100000_fix_schema.sql   # UNIQUE constraints, CASCADE, confirmed status
    └── functions/
